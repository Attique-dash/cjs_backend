...
export const authenticateSimple = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Check multiple possible header locations for the token (NO BEARER REQUIREMENT)
    let token: string | undefined;

    // 1. Check Authorization header (accept token with or without Bearer prefix)
    const authHeader = req.headers.authorization;
    if (authHeader) {
      const authHeaderStr = Array.isArray(authHeader) ? authHeader[0] : authHeader;
      
      // Remove Bearer prefix if present, but also accept token without it
      if (authHeaderStr.startsWith('Bearer ') || authHeaderStr.startsWith('bearer ')) {
        token = authHeaderStr.substring(7).trim();
      } else {
        token = authHeaderStr.trim(); // Accept token without Bearer
      }
    }
    ...
    logger.info('Simple auth middleware - Token found:', !!token);
    ...
    if (!token || token.length === 0) {
      res.status(401).json({
        ...
        error: 'Missing token',
        hint: 'Include token in Authorization header (format: Authorization: <token>), x-auth-token header, or ?token= query parameter',
    ...
    req.user = user;
    logger.info(`User authenticated via simple auth: ${user.email} (${user.role})`);
    next();

  } catch (error: any) {
    logger.error('Simple authentication error:', error);
