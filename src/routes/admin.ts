 ...
 *   post:
 *     summary: Generate API Key for KCD Logistics
 *     description: Generates a permanent API key specifically for KCD Logistics. This key is shown only once in the response.
 *     tags: [Admin]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *               name:
 *                 type: string
 *                 example: KCD Logistics Webhook
 *               permissions:
 *                 type: array
 *                 items:
 *                   type: string
 *                 example: ["kcd_webhook", "webhook", "all"]
 *               description:
 *                 type: string
 *                 example: API key for KCD Logistics packing system
 *               warehouseId:
 *                 type: string
 *                 example: 507f1f77bcf86cd799439011
 *             required:
 ...
 *                     key:
 *                       type: string
 *                       example: kcd_alb2c3d4e5f67890abcdef...
 *                     name:
 *                       type: string
 *                       example: KCD Logistics Webhook
 *                     permissions:
...
router.post('/api-keys/kcd', 
  authenticate, 
  authorize('admin'), 
  asyncHandler(adminController.generateKCDApiKey)
);
